...
export function sessionSecret() {
  ...
  if (process.env.VERCEL) {
    throw new Error("Defina SESSION_SECRET na Vercel.");
  }
  ensureDir();
...
function adminPassword() {
  ...
  assertNotPlaceholder("ADMIN_PASSWORD", password);
  return password;
}

function staffAccount() {
  const password = process.env.ALEX_PASSWORD || "";
  if (password) assertNotPlaceholder("ALEX_PASSWORD", password);
  return {
...
async function syncUsers(sql) {
  ...
  ensureDir();
  const existing = fs.existsSync(usersFile) ? JSON.parse(fs.readFileSync(usersFile, "utf8")) : [];
